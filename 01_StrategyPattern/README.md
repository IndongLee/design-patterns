...
- Duck이라는 수퍼 클래스에 fly() 메소드가 추가되면서 fly()가 있어서는 안 되는 RubberDuck 서브 클래스까지 상속을 받게 된다.
...
- 서브 클래스가 추가될 때마다 일일이 quack()과 fly() 메소드를 오버라이드 해야 하는 불편함이 발생한다.
...
- 코틀린은 인터페이스에서 메소드에 구현 내용을 포함할 수 있다. 따라서 이러한 문제를 어느 정도 해결할 수 있다.
...
- 이러한 행동을 Duck 클래스로부터 갈라내기 위해서 두 메소드를 모두 Duck 클래스로부터 끄집어내서 각 행동을 나타낼 클래스 집합을 새로 만든다.
...
이제 Duck에서 나는 행동과 소리를 내는 행동을 Duck 클래스(또는 그 서브 클래스)에서 정의한 메소드를 써서 구현하지 않고 다른 클래스에 `위임`한다.
